...
if [ $# -ne 7 ]; then
	echo "Usage: $0 [resultsdir] [allquerylog] [report] [teamidnotfoundlog] <nqueries> <teamsize> <clusternodes>"
	exit 1;
fi
...
while [ $n -lt $NQUERIES ]; do
	...
	#ret=`grep "queryresult: teamID NOT found qid: $n " $RESULTSDIR/*.$TYPE > $TMPFILE; echo $?`
	#ret=`grep "queryresult: teamID found qid: $n " $RESULTSDIR/*.$TYPE > $TMPFILE; echo $?`
	ret=`grep -l "qid: $n " $RESULTSDIR/*.TYPE > $TMPFILE; echo $?`
	#queryres=0
	#queryres=`grep "queryresult: teamID found qid: $n " $RESULTSDIR/*.$TYPE`
	#if [ -z "$queryres" ]; then
	if [ $ret -ne 0 ]; then
		echo "skipping qid: $n"
		n=`expr $n + 1`
