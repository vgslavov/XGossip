...
# was 17
if [ $# -ne 12 ]; then
	echo "Usage: start_gpsi.sh <bcast|sigquery|xquery|vanilla|xinit|xexec> <total peers> <cluster nodes> <gossip interval> <listen time> <wait time> <querydir|sigdir|junk> <teamsize> <k> <l> <compress:0|1> [rounds] <churn peers> <boot port> <sessions file> <killed file> <kill round range>"
	exit 1;
fi
...
while [ $N -gt 0 ] ; do
	...
	# xgossip init
		...
		$GPSIPATH -S $TMP/$N/dhash-sock -G $TMP/$N/g-sock -L $TMP/$N/log.gpsi -s $DATADIR/$N/ -g -t $GINTVAL -T $INSERTTIME -w $WAITTIME -W $LISTENTIME -H -d $PRIME -j $HOME/etc/irrpoly-deg9.dat -I -p -P $TMP/$N/log.init -B $BANDS -R $ROWS -F $TMP/$N/hash.dat -Z $TEAMSIZE -q $PEERS -N $CLUSTERNODES -K $KILLRANGE -f $KILLFILE &
	# xgossip exec
	elif [ $XEXEC = 1 ] ; then
		if [ $COMPRESS = 1 ] ; then
			$GPSIPATH -S $TMP/$N/dhash-sock -G $TMP/$N/g-sock -L $TMP/$N/log.gpsi -g -t $GINTVAL -T $INSERTTIME -w $WAITTIME -W $LISTENTIME -H -d $PRIME -j $HOME/etc/irrpoly-deg9.dat -E -p -P $TMP/$N/log.init -B $BANDS -R $ROWS -F $TMP/$N/hash.dat -Z $TEAMSIZE -q $PEERS -o $ROUNDS -N $CLUSTERNODES -C -K $KILLRANGE -f $KILLFILE &
		else
...
# churn peers
N=$CHURNPEERS
TMP="$HOME/tmp.churn.gpsi"
echo "starting $N churn peers..."
for i in `cat $SESSIONSFILE`; do
	mkdir $TMP/$N
	echo $i
	$GPSIPATH -c -L $TMP/$N/log.gpsi -t $GINTVAL -A $BOOTPORT -W $LISTENTIME -q $PEERS -o $ROUNDS -k $i -N $CLUSTERNODES &
	N=`expr $N - 1`
done
