...
# does this show primary/nearest?
#grep 'Selected replica id' $SIGDIR/$IPADDR.$CURRTIME.queryfile >> $TMPFILE
...
# the same as query time if one query per psi
#grep 'Time taken' $SIGDIR/$IPADDR.$CURRTIME.queryfile >> $TMPFILE
...
#grep 'Docid' $SIGDIR/$IPADDR.$CURRTIME.queryfile >> $RESULTS
